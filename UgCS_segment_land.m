function Z = UgCS_segment_land(docNode1,data)
% This function appends a "Landing" segment onto a UgCS xml file
% This is only applicable to certain UAVs
% ******** Do not used on fixedwing platforms ********
% inputs-
%       - docNode1 - xml document being appended
%       - data      - 7x2 cell

    order = data{1,2};
    lat = data{2,2}*pi/180;    % DD is converted to radians
    long = data{3,2}*pi/180;   % DD is converted to radians
    altitude = data{4,2};   % m
    %speed = data{5,2};      % m/s
    obstacles = data{5,2};  % True or False
    if obstacles==0
        obstacles='False';
    else
        obstacles='True';
    end
    terrain = data{6,2};    % True or False
        if terrain==0
        terrain='False';
    else
        terrain='True';
         ...
         route_node.appendChild(segments_node);
            ...
            algorithmClassName_node.setAttribute('v','com.ugcs.ucs.service.routing.impl.LandingAlgorithm');
            ...
            segments_node.appendChild(figure_node);
                ...
                type_node.setAttribute('v','LANDING_POINT');
                ...
                figure_node.appendChild(ugcsList_node); 
                    ...
                    o_node.setAttribute('v3',num2str(lat,16));  
                    o_node.setAttribute('v4',num2str(long,16));  
