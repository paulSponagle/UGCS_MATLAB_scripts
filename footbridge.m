...
image_angle = 190;      % +/- 65degrees
swaths = 25;            % whole number
distance = 9;          % distance from target
speed = 1.5;              % m/s
...
% capability)
%safe_long = ;            %
start_lat = 43.0947321;   % DD - pick start point before the structure, but aligned with it.
start_long = -77.4292473; % DD
start_height = 8;         % metres

endLat = 43.0949371;
endLong = -77.4290949;
%direction = 27;         % degrees - orientation of span.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
span = 25;               % metres
direction = 28.5;        % degrees - orientation of span.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
homeLong=start_long;
interval = 7; % metres
routeName='Elevated Span (footbridge) Fairport';
[midLat,midLong] = newPosition(start_lat, start_long, direction, span/2); % useful if POI action is used
%[endLat,endLong] = newPosition(start_lat, start_long, direction, span); %useful if yaw action is used.
%% WP calculation
...
for ii=1:size(WP,1)
    ...
    % Add yaw angle action
    if ii<size(WP,1)
        seq= mod(ii,4); % ii corresponds to Waypoint 0
        
        if seq==0 || seq==1
            targetLat=start_lat;
            targetLong=start_long;
        else
            targetLat = endLat;
            targetLong = endLong;
        end
        nextWPBearing=bearing(WP(ii,1),WP(ii,2),WP(ii+1,1),WP(ii+1,2));
        targetBearing=bearing(WP(ii,1),WP(ii,2),targetLat, targetLong);
        yawAngle=floor(targetBearing-nextWPBearing);
        yawAngle(yawAngle<0)=yawAngle+360;
        yawAngle(yawAngle>=360)=yawAngle-360;
        yaw_data{1,1}='order';
        yaw_data{2,1}='heading';
        yaw_data{1,2}=act_order;
        yaw_data{2,2}=yawAngle;
        UgCS_Action_Yaw(docNode1,yaw_data,seg_order);
    end
